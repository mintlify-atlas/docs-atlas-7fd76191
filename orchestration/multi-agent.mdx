...
description: Build parent-child agent hierarchies for parallel and distributed workflows
---

Multi-agent orchestration in Jido follows a simple pattern: parent agents spawn children via directives, send work, and aggregate results. No manual process management required.
...
## Building a Parallel URL Fetcher

Let's build a coordinator that spawns worker agents to fetch multiple URLs in parallel, then aggregates the results.
...
```elixir lib/parallel_fetcher.ex
...
- Use `SpawnAgent` directive to create child agents
- Handle `jido.agent.child.started` to initialize children
- Use `emit_to_pid` to send work and `emit_to_parent` to report results
- Track work in coordinator state (pending/completed maps)
- Use `await` primitives for synchronous coordination
- Handle child failures via `jido.agent.child.exit` signals

## Next Steps

- [Coordination Primitives](/orchestration/coordination) - Event-driven waiting and cancellation
- [Scheduling](/orchestration/scheduling) - Delayed and recurring work
