...
description: Event-driven waiting, cancellation, and synchronization patterns for async agents
---

Jido provides coordination primitives for waiting on async agents without polling or guesswork. Agents signal completion via state, and callers block until ready.
...
<Note>
A `:failed` status from an agent is still success from the coordinator's perspective—it completed. Infrastructure errors (process death, network issues) return the error tuple.
</Note>
...
- [Multi-Agent Patterns](/orchestration/multi-agent) - Parent-child hierarchies
