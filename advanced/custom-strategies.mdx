...
description: Implement custom execution strategies for specialized control flow
---

Strategies control how agents execute instructions. While the built-in `Direct` and `FSM` strategies handle most use cases, you can implement custom strategies for specialized execution patterns.
...
Here's a complete example that executes one instruction per call, rotating through them:
...
Here's the minimal boilerplate for a custom strategy:
...
<CardGroup cols={2}>
  <Card title="FSM Strategy" icon="diagram-project" href="/advanced/fsm-strategy">
    Learn the FSM strategy implementation
  </Card>
  <Card title="Error Handling" icon="circle-exclamation" href="/advanced/error-handling">
    Handle errors in custom strategies
  </Card>
</CardGroup>
